...
**当前服务端实现**：仅返回 201/400/500，插件应优雅降级处理
...
**未来服务端增强**（插件应预留支持）：
- **408 超时**：视为临时故障，触发 1s/2s/5s 重试
- **429 限流**：指数退避（60s/120s/300s），最多3次
- **503 服务不可用**：立即缓存到 IndexedDB，30秒后重试
...
**注意**：
- vector 字段由服务端自动生成，插件无需提供
- 当前服务端仅实现 201/400/500 响应
- 插件应优雅处理未来可能新增的 408/429/503 响应
